"""Add listing image_url and expose it in analytics views.
...
def upgrade() -> None:
    _drop_views()
    op.add_column("listings", sa.Column("image_url", sa.Text()))
    _create_views()
...
def downgrade() -> None:
    ...
    previous = __import__(
        "db.migrations.versions.006_investigation_matching",
        fromlist=["_create_views", "_drop_views"],
    )
    previous._drop_views()
    previous._create_views()
